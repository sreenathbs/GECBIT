...
$rows = "RegisterNumber 	BE100 	BE102 	CS100 	CS120 	CY100 	CY110 	EE100 	EE110 	MA102 	.../240	sgpa	Percentage
...
// sql to create table
$sql = "CREATE TABLE result_s$semester (
`id` INT(5) PRIMARY KEY AUTO_INCREMENT,
`RegisterNumber` VARCHAR(12),";
foreach ($cols as $column_name) {
	$sql .= "`$column_name` VARCHAR(10) NOT NULL,";
}
$sql .="`mod_date` TIMESTAMP)";
...
switch (count($params[0])) {
    case 14:
        for($i=0; $i<count($rows); $i++) {
			$sql = $db->prepare("INSERT INTO `result_s$semester`($op) VALUES($qm)");
			$sql->bind_param($s, $params[$i][0], $params[$i][1], $params[$i][2], $params[$i][3], $params[$i][4], $params[$i][5], $params[$i][6], $params[$i][7], $params[$i][8], $params[$i][9], $params[$i][10], $params[$i][11], $params[$i][12], $params[$i][13]);
			$sql->execute();
		}
        break;
    case 13:
        for($i=0; $i<count($rows); $i++) {
			$sql = $db->prepare("INSERT INTO `result_s$semester`($op) VALUES($qm)");
			$sql->bind_param($s, $params[$i][0], $params[$i][1], $params[$i][2], $params[$i][3], $params[$i][4], $params[$i][5], $params[$i][6], $params[$i][7], $params[$i][8], $params[$i][9], $params[$i][10], $params[$i][11], $params[$i][12]);
			$sql->execute();
		}
        break;
    case 12:
        for($i=0; $i<count($rows); $i++) {
			$sql = $db->prepare("INSERT INTO `result_s$semester`($op) VALUES($qm)");
			$sql->bind_param($s, $params[$i][0], $params[$i][1], $params[$i][2], $params[$i][3], $params[$i][4], $params[$i][5], $params[$i][6], $params[$i][7], $params[$i][8], $params[$i][9], $params[$i][10], $params[$i][11]);
			$sql->execute();
		}
        break;
    case 11:
        for($i=0; $i<count($rows); $i++) {
			$sql = $db->prepare("INSERT INTO `result_s$semester`($op) VALUES($qm)");
			$sql->bind_param($s, $params[$i][0], $params[$i][1], $params[$i][2], $params[$i][3], $params[$i][4], $params[$i][5], $params[$i][6], $params[$i][7], $params[$i][8], $params[$i][9], $params[$i][10]);
			$sql->execute();
		}
        break;
    case 10:
        for($i=0; $i<count($rows); $i++) {
			$sql = $db->prepare("INSERT INTO `result_s$semester`($op) VALUES($qm)");
			$sql->bind_param($s, $params[$i][0], $params[$i][1], $params[$i][2], $params[$i][3], $params[$i][4], $params[$i][5], $params[$i][6], $params[$i][7], $params[$i][8], $params[$i][9]);
			$sql->execute();
		}
        break;
    case 9:
        for($i=0; $i<count($rows); $i++) {
			$sql = $db->prepare("INSERT INTO `result_s$semester`($op) VALUES($qm)");
			$sql->bind_param($s, $params[$i][0], $params[$i][1], $params[$i][2], $params[$i][3], $params[$i][4], $params[$i][5], $params[$i][6], $params[$i][7], $params[$i][8]);
			$sql->execute();
		}
        break;
    case 8:
        for($i=0; $i<count($rows); $i++) {
			$sql = $db->prepare("INSERT INTO `result_s$semester`($op) VALUES($qm)");
			$sql->bind_param($s, $params[$i][0], $params[$i][1], $params[$i][2], $params[$i][3], $params[$i][4], $params[$i][5], $params[$i][6], $params[$i][7]);
			$sql->execute();
		}
        break;
    default:
